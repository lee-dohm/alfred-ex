defmodule Alfred.ResultList.Spec do
  ...
  describe "construction" do
    ...
    it "creates a list with variables" do
      ...
      list = ResultList.new([item, item, item], variables: variables)
    ...
    it "creates a list with a rerun value" do
      item = Result.new("title", "subtitle")
      variables = %{foo: "bar"}
      list = ResultList.new([item, item, item], variables: variables, rerun: 3.0)
    ...
    it "throws an error if the rerun value is not a number" do
      ...
      expect fn -> ResultList.new([item], variables: %{}, rerun: "foo") end |> to(raise_exception ArgumentError)
    end

    it "throws an error if the rerun value is less than 1.0 or greater than 5.0" do
      item = Result.new("title", "subtitle")

      expect fn -> ResultList.new([item], variables: %{}, rerun: -1.0) end |> to(raise_exception ArgumentError)
      expect fn -> ResultList.new([item], variables: %{}, rerun: 10.0) end |> to(raise_exception ArgumentError)
    end
  end
